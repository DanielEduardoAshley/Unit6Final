...
showsServices.readByShowid=(id)=>{
    return dbConn(dbUrl).any('SELECT * FROM shows JOIN comments ON show_id=shows.id JOIN users ON users.id=comments.user_id JOIN genres ON genre_id=genres.id WHERE shows.id=$[id]', {id})
};
